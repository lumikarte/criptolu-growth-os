...
def iso(tmp_path, monkeypatch):
    ...
    up, tr, cl, ex, tmp, jb = (
        ...
        tmp_path / "clips", tmp_path / "exports", tmp_path / ".tmp", tmp_path / "jobs",
    )
    for d in (up, tr, cl, ex, tmp, jb):
        d.mkdir()
    ...
    monkeypatch.setattr(config, "JOBS_DIR", jb)  # CRI-603: aislar los jobs por test
